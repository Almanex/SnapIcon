...
* **Слой заднего плана (`ic_launcher_background.png`):**
  * Вы можете выбрать цвет фонда, введя Hex-код (например, `#FFFFFF` или `#3DDC84`), выбрав готовый цвет из быстрых палитр (свотчей) или указав файл с пользовательским фоновым изображением (текстурой или паттерном).
